...
#from tensorflow.python.compiler.tensorrt import trt #new line here
...
def get_func_from_saved_model(saved_model_dir):
  ...
  #    export_dir=saved_model_dir, tags=[tag_constants.SERVING])
  ...
  #builder.save()
  #end adding for tag
  #export_dir = 'tmp_resnet101'
  #builder = tf.compat.v1.saved_model.Builder(export_dir)
  #with tf.Session(graph=tf.Graph()) as sess:
  #builder.add_meta_graph_and_variables([tf.saved_model.tag_constants.SERVING],)
  #with tf.Session(graph=tf.Graph()) as sess:
  #builder.add_meta_graph(["serve"])
  #builder.save()


  saved_model_loaded =  tf.compat.v1.saved_model.load_v2(saved_model_dir, tags=[tag_constants.SERVING])
...
def get_graph_func(input_saved_model_dir,
  ...
  if use_trt:
    ...
    if conversion_params.precision_mode != 'INT8':
      ...
      print('\n')
      print(calib_files)
      print(num_calib_inputs//batch_size)
...
def eval_fn(preds, labels, adjust):
  ...
  preds = np.argmax(preds).reshape(-1) - adjust
  ...
  if mode == 'validation':
    ...
    for i, batch_images in enumerate(dataset):
      if i >= num_warmup_iterations:
        start_time = time.time()
        batch_preds = list(graph_func(batch_images))[0].numpy()
        #print(type(graph_func(batch_images)))
        #batch_preds = graph_func(batch_images)[0].numpy()
        #batch_preds = list(graph_func(batch_images).values())[0].numpy()
        # what i thought it should be! batch_preds = list(graph_func(batch_images))[0].numpy()
        #batch_preds = list(tf.convert_to_tensor(graph_func(batch_images)[0].numpy()))
        #batch_preds = graph_func(batch_images)[0].values().numpy()
        #batch_preds = list(graph_func(batch_images).values())[0].numpy()
        ...
        batch_preds = list(graph_func(batch_images))[0].numpy()
        #batch_preds = graph_func(batch_images)[0].values().numpy()
        #batch_preds = graph_func(batch_images)[0]
...
if __name__ == '__main__':
  ...
  print('CALIB DATA DIR*******')
  print(args.calib_data_dir)
  print(args.data_dir)
  ...
  if not args.use_synthetic:
    ...
    if args.precision == 'INT8':
      print('ASKING FOR INT8')
    #changed the following to validation* from train*    
      calib_files = get_files(args.calib_data_dir, 'train*')
      print('CALIB FILES FROM LINE 467')
  #added this here
  calib_files = get_files(args.calib_data_dir, 'train*')
  print('DONE CHECKING PRECISION')
  config_gpu_memory(args.gpu_mem_cap)
  print('PASSED GET_FILES')
